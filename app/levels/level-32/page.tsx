...
/**
 * LEVEL-32 QUESTION BANK
 * 
 * Video-based observation questions testing attention to detail
 * Based on the video content provided by the user
 */
const questions: Question[] = [
  {
    id: 1,
    question: "What is the first word that appears on the screen?",
    options: ["Google", "Search", "Welcome", "Hello"],
    correct: "Google"
  },
  {
    id: 2,
    question: "How many clocks appeared in the video?",
    options: ["10", "11", "12", "13"],
    correct: "12"
  },
  {
    id: 3,
    question: "Which news channel is shown in the video?",
    options: ["Manorama news", "Asianet News", "CNN News", "BBC News"],
    correct: "Manorama news"
  },
  {
    id: 4,
    question: "Which actress is seen giving a speech in the video?",
    options: ["Kalpana", "Revathi", "Shobana", "Urvashi"],
    correct: "Kalpana"
  },
  {
    id: 5,
    question: "What is the dog's name shown in the video?",
    options: ["Snoopy", "Scooby", "Buddy", "Charlie"],
    correct: "Snoopy"
  }
];

export default function Level32Page() {
  ...
  const [canStartQuestions, setCanStartQuestions] = useState<boolean>(false);
  ...
  const fetchTeamData = useCallback(async (teamCode: string) => {
    try {
      ...
      // Check if video was already watched in this session
      const videoWatchedStatus = sessionStorage.getItem('level32_video_watched');
      if (videoWatchedStatus === 'true') {
  ...
  const handleVideoComplete = () => {
    ...
    setCanStartQuestions(true);
    setShowVideoModal(false);
    sessionStorage.setItem('level32_video_watched', 'true');
    toast.success("Video completed! Now answer the questions based on what you observed.");
  };
  ...
  /**
   * ENHANCED SCORING ALGORITHM FOR LEVEL-32
   *
   * Video-based level scoring with emphasis on observation skills
   */
  const calculateScore = (completionTime?: number): {
    ...
    // Base scoring calculation - higher points for video observation questions
    const baseScore = levelStats.correct * 1800; // Higher base score for observation skills

    // Penalties
    const penalties = (levelStats.incorrect * 500) + (levelStats.skipped * 900);
    ...
    let timeBonus = 0;
    if (timeTaken < 2) timeBonus = 400;
    else if (timeTaken < 3) timeBonus = 350;
    else if (timeTaken < 4) timeBonus = 300;
    else if (timeTaken < 5) timeBonus = 250;
    else if (timeTaken < 6) timeBonus = 200;
    else if (timeTaken < 7) timeBonus = 150;
    else if (timeTaken < 8) timeBonus = 100;
    else if (timeTaken < 9) timeBonus = 50;
  ...
  const completeLevel = async () => {
    ...
    try {
      ...
      setIsCompleted(true);
      // Clear video watched status for next game session
      sessionStorage.removeItem('level32_video_watched');
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-600">Loading Level 32...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50 flex items-center justify-center p-4">
        <Card className="max-w-4xl mx-auto">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-3xl font-bold text-green-700">Level 32 Complete!</CardTitle>
            <div className="mt-2">
          ...
          <CardContent className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              ...
              <div className="p-4 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">{scoreData.accuracy.toFixed(1)}%</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="text-center p-4 bg-indigo-50 rounded-lg">
                <div className="flex items-center justify-center mb-2">
                  ...
                  <span className="text-lg font-semibold text-indigo-700">Observation Skills</span>
                </div>
                <div className="text-2xl font-bold text-indigo-600">
                  {scoreData.performanceRating}
                </div>
                <div className="text-sm text-indigo-600">video analysis</div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-gray-700">Base Score (Video Observation)</span>
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              ...
              <p className="text-sm text-gray-600">
                {scoreData.performanceRating === 'Excellent' &&
                  "Outstanding observation skills! You caught all the important details in the video. Excellent work!"}
                {scoreData.performanceRating === 'Good' &&
                  "Great attention to detail! You demonstrated solid observation skills. Well done!"}
                {scoreData.performanceRating === 'Average' &&
                  "Good effort! Your observation skills are developing. Try to focus on more details in future video challenges."}
                {scoreData.performanceRating === 'Needs Improvement' &&
                  "Keep practicing your observation skills! Watch videos more carefully and try to notice small details."}
              </p>
            </div>
  ...
  // Show waiting screen if video hasn't been watched yet
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50">
        ...
        <div className="flex items-center justify-center min-h-screen">
          <Card className="max-w-md mx-auto text-center">
            <CardHeader>
              <CardTitle className="text-2xl font-bold text-purple-700">Level 32 - Video Challenge</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="text-gray-600">
                {!videoWatched ? (
                  <p>Please watch the video first to proceed with the questions.</p>
                ) : (
                  <p>Video completed! Loading questions...</p>
                )}
              </div>
              {videoWatched && (
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-center p-3 bg-green-50 border border-green-200 rounded-lg">
              ...
              <span className="text-green-700 font-medium">Video watched - Answer based on your observations</span>
            </div>
          </div>
